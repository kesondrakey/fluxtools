...
#Titles and things----
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      bslib::accordion(
        id = "advanced_box",
        open = character(0),  # keep closed by default - doesnt work
        bslib::accordion_panel(
          title = tagList(icon("sliders-h"), "Advanced (Set Timezone)"),
    ...
    hr(),
      ...
      bslib::accordion(
        ...
        bslib::accordion(
          ...
          open = character(0),  # keep closed by default
          bslib::accordion_panel(
            title = tagList(icon("sliders-h"), "Time subset"),
            value = "advanced",  # <-- REQUIRED: unique character id
...
server <- function(input, output, session) {
  ...
  .current_phase_icons <- function(hh_vec, invert = FALSE) {
    ...
    day    <- .hod_in_range(hrs, 6, 18)   # 06:00–18:00
    day_on <- any(sel & day)
    nite_on<- any(sel & !day)
