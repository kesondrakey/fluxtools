...
server <- function(input, output, session) {
  ...
  selected_ts <- reactive({
    sel <- tryCatch(plotly::event_data("plotly_selected", source = "qc_plot"),
                    error = function(e) NULL)
    if (is.null(sel) || !nrow(sel)) return(character(0))

    k <- as.character(sel$key)
    k <- k[!is.na(k) & nzchar(k)]
    # If overlay keys like "YYYYMMDDHHMM||VAR", keep only the timestamp part
    k <- sub("^(.*)\\|\\|.*$", "\\1", k)
    unique(k)
  })
...
#   })
  ...
  selected_pairs <- reactive({
    ...
    if (!length(k)) return(data.frame(ts=character(0), var=character(0)))

    if (all(grepl("\\|\\|", k))) {
      # ✅ use REGEX split (no fixed=TRUE here)
      parts <- strsplit(k, "\\|\\|")
      df <- data.frame(
    ...
    # sanitize + keep only variables that exist in the data
  ...
  observeEvent(input$remove, {
    ...
    # Vars we’re allowed to edit right now (overlay-aware)
    allowed <- intersect(vars_to_edit(), names(rv$df) %||% character())

    # If you brushed something, treat that as authoritative. Otherwise, use staged.
    vars_all <- if (nrow(pairs)) intersect(unique(pairs$var), allowed)
    else               intersect(names(staged),     allowed)

    if (!length(vars_all)) {
      showNotification("Nothing to remove for current selection.", type = "message"); return()
    }
