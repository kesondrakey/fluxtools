...
server <- function(input, output, session) {
  ...
  vars_to_edit <- reactive({
    if (isTRUE(input$overlay_mode) && length(input$overlay_vars)) {
  ...
  selected_ts <- reactive({
    sel <- tryCatch(plotly::event_data("plotly_selected", source = "qc_plot"),
                    error = function(e) NULL)
    if (is.null(sel) || !nrow(sel)) character(0) else unique(sel$key)
  })
  ...
  observeEvent(input$add_sel, {
    pairs <- selected_pairs(); if (!nrow(pairs)) return()

    # keep orange layer in sync (rows are fine, for preview/table)
    rows <- df_by_year()$.row[df_by_year()$ts_str %in% pairs$ts]
    sel_keys(unique(c(isolate(sel_keys()), rows)))

    # add per-variable
    for (v in unique(pairs$var)) {
    ...
    showNotification(
      sprintf("Flagged %d timestamp(s) across %d variable(s).",
              length(unique(pairs$ts)), length(unique(pairs$var))),
  ...
  raw_df <- reactive({
    ...
    read.csv(
      ...
      na.strings = c("NA", "NaN", "", "-9999", "-9999.0")
    )
  })
...
#Tooltip thats automatic and obvious (doesnt need mouse hover)
  ...
  # observeEvent(input$add_sel, {
  #   ts <- selected_ts()                 # <- returns ts_str
  #   if (!length(ts)) return()
  #
  #   # keep orange “accumulated” rows in sync (uses .row)
  #   rows <- df_by_year()$.row[df_by_year()$ts_str %in% ts]
  #   sel_keys(unique(c(isolate(sel_keys()), rows)))
  #
  #   # add timestamps to each variable’s staging set
  #   for (v in vars_to_edit()) {
  #     old <- removed_ts[[v]] %||% character()
  #     removed_ts[[v]] <- unique(c(old, ts))
  #   }
...
#   })
  ...
  selected_pairs <- reactive({
    sel <- tryCatch(plotly::event_data("plotly_selected", source = "qc_plot"), error = function(e) NULL)
    if (is.null(sel) || !nrow(sel)) return(data.frame(ts=character(0), var=character(0)))
    k <- unique(as.character(sel$key))
    if (all(grepl("\\|\\|", k))) {
      ...
      data.frame(ts = unique(k), var = rep(input$yvar, length(unique(k))), stringsAsFactors = FALSE)
    }
  })
  ...
  observeEvent(time_rng_debounced(), ignoreInit = TRUE, {
    tr <- time_rng_debounced()
    s_view <- to_view_time(tr[1])
    e_view <- to_view_time(tr[2])

    # only push if different to avoid loops
  ...
  observeEvent(input$remove, {
    ...
    # union staged + current selection per var
    vars_all <- unique(c(names(staged), unique(pairs$var)))
    if (!length(vars_all)) return()

    tmp <- rv$df
    total_tses <- 0L

    for (v in vars_all) {
      ts_v <- unique(c(staged[[v]] %||% character(),
                       pairs$ts[pairs$var == v]))
      if (!length(ts_v) || !is.numeric(tmp[[v]])) next

      rows_global <- tmp$.row[tmp$ts_str %in% ts_v]
      tmp[[v]][match(rows_global, tmp$.row)] <- NA_real_

      old <- confirmed_ts[[v]] %||% character()
      confirmed_ts[[v]] <- unique(c(old, ts_v))
